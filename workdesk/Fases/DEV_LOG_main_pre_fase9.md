# DEV_LOG da `main` — registro anterior à Fase 9

> 🗂️ **Arquivo de resgate.** Este é o `DEV_LOG.md` como ele existia na branch
> `main` até 16/08/2026, quando a `main` foi alinhada com a `staging`.
>
> **Por que ele está aqui:** a `main` nunca recebeu a reorganização da workdesk
> da Fase 9, e seguiu recebendo entradas próprias (março a abril de 2026, mais
> os fixes de produção de agosto). Essas entradas **não estavam em
> `Fases/`** — uma lacuna de arquivamento anterior a este trabalho, não criada
> por ele. Sem esta cópia, elas sumiriam da árvore no momento em que a `main`
> fosse alinhada, sobrevivendo só dentro do histórico do git, onde ninguém
> pensaria em procurar.
>
> Não editar. É retrato, não documento vivo.
